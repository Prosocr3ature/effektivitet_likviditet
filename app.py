...
with tab3:
    ...
    if not df3.empty:
        ...
        prompt = (
            f"Veckorapport: vecka {week_num}, totalt TB "
            f"{tot_tb:.0f} kr över {days} dagar. "
            "Ge en peppande sammanfattning på svenska."
        )
        if openai_client:
            try:
            ...
            st.info(
                'Ange OPENAI_KEY i secrets för GPT-summering'
            )
